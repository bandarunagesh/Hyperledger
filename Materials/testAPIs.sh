...
function setChaincodePath(){
	...
	case "$LANGUAGE" in
		...
		;;
		"node")
		CC_SRC_PATH="$PWD/artifacts/src/github.com/reconpoc/node"
		;;
...
TRX_ID=$(curl -s -X POST \
  http://localhost:4000/channels/mychannel/chaincodes/recon \
  -H "authorization: Bearer $ORG1_TOKEN" \
  -H "content-type: application/json" \
  -d '{
	"peers": ["peer0.org1.example.com","peer0.org2.example.com"],
	"fcn":"createApplication",
	"args":["ASK_111","Test Application","Dev","Test","","","","","","","","","","","","","","",""]
}')
...
echo "GET range query chaincode on peer1 of Org1"
echo
curl -s -X GET \
  "http://localhost:4000/channels/mychannel/chaincodes/recon?peer=peer0.org1.example.com&fcn=getApplication&args=%5B%22Application%22%5D" \
  -H "authorization: Bearer $ORG1_TOKEN" \
  -H "content-type: application/json"
echo
echo
